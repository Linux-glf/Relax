...
#
# Qwen35-9B 8xNPU colocate training script.
#
# Usage:
#   bash scripts/training/text/run-qwen35-9B-8xnpu-colocate.sh
...
mkdir -p log
    ...
    "${EVAL_ARGS[@]}" \
    "${SGLANG_ARGS[@]}" \
    "${MISC_ARGS[@]}" 2>&1 | tee log/qwen35-9B-GRPO-npu8-colocate-${now}.log
